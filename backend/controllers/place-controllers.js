...
const deletePlace = async (req, res, next) => {
    ...
    try {
        ...
        return res.status(500).json({ message: 'Semthing went wrong, could not find place with that id'})
    }
    
    try {
        await place.deleteOne()
    } catch (error) {
        return res.status(500).json({ message: 'Semthing went wrong could not delete place!'})
    }
